...
data_gen <- function(maxunif,
  ...
  # kappa <- sqrt(8) / param_vals$rho
  # tau = 1 / (sqrt(4*pi) * sqrt(param_vals$sigma2) * kappa)
  # 
  # mesh_sim <- fm_mesh_2d(boundary = areas,
  #                        max.edge = c(.03, 0.1),
  #                        offset = c(0.17, 0.2))
  # matern_sim <- inla.spde2.pcmatern(mesh_sim,
  #                                   prior.sigma = c(1, 0.5),
  #                                   prior.range = c(0.5, 0.5))
  # Q <- inla.spde2.precision(spde = matern_sim,
  #                           theta = c(log(kappa),log(tau))) 
  # matern_field <- inla.qsample(n = 1,
  #                              Q = Q,
  #                              mu = rep(0, dim(Q)[1]))
  # 
  # loc <- st_coordinates(st_centroid(nested_grids_sf)) %>%
  #   as.data.frame() %>%
  #   st_as_sf(coords = c("X", "Y"), crs = st_crs(nested_grids_sf))
  # A_field <- inla.spde.make.A(mesh = mesh_sim, loc = loc)
  # nested_grids_sf$field <- as.vector(A_field %*% matern_field)
  # 
  
  
  # Compute the latent mu values at the small grids (b)
  
  nested_grids_sf$log.mu <- param_vals$beta0 + nested_grids_sf$covariate * param_vals$beta1 + nested_grids_sf$field
  nested_grids_sf$mu <- exp(nested_grids_sf$log.mu)
  ...
  if(samp_prop == 1){
    ...
    set.seed(Sys.time())
    data_Y$y <- rpois(n = nrow(data_Y), lambda = data_Y$mu)
...
fit_block_approach <- function(data = sim_data){
  ...
  agg <- bru_mapper_logsumexp(rescale = FALSE, n_block = nrow(data$data_Y))
  ...
  fit_block <- bru(cmp,
                   bru_obs(
                     y ~ ibm_eval(
                       mapper = agg,
                       input = list(weights = weight, block = block),
                       state = intercept + covariate + sp_field
                     ),
                     family = "poisson",
                     data = integration_data,
                     response_data = data$data_Y,
...
compute_proper_scores <- function(data = sim_data,
  ...
  if(block == FALSE){
    ...
    pred_vals <- predict(fit_res$res, data_Y_centroid,
                         formula = ~ {
                           E <- exp(intercept + covariate)
                           list(
                             expeta = E,
                             dens = dpois(data_Y_centroid$y, lambda = E)
                           )
                         },
                         n.samples = 2000
    )
    post_E <- pred_vals$expeta$mean
    post_Var <- pred_vals$expeta$mean + pred_vals$expeta$sd^2
    scores <- data.frame(
    ...
    pred_vals <- predict(fit_res$res, fit_res$integration_data,
                         formula = ~ {
                           E <- exp(ibm_eval(mapper = fit_res$agg,
                                             input = list(weights = weight, block = block),
                                             state = intercept + covariate))
                           list(
                             expeta = E,
                             dens = dpois(data$data_Y$y, lambda = E)
                           )
                         },
                         n.samples = 2000
    )
...
compute_proper_scores_spatial <- function(data = sim_data,
  ...
  if(block == FALSE){
    ...
    pred_vals <- predict(fit_res$res, data_Y_centroid,
                         formula = ~ {
                           E <- exp(intercept + covariate + sp_field)
                           list(
                             expeta = E,
                             dens = dpois(data_Y_centroid$y, lambda = E)
                           )
...
compute_proper_scores_mrf <- function(shp = sim_data$areas_C,
  ...
  pred_vals <- predict(fit_res$res, data_Y_centroid,
                       formula = ~ {
                         E <- exp(intercept + covariate + bym2)
                         list(
                           expeta = E,
                           dens = dpois(data_Y_centroid$y, lambda = E)
                         )
...
compute_other_scores <- function(data = sim_data,
  ...
  if(block == FALSE){
    ...
    pred_mu_b <- data.frame(block = 1:nrow(pred_vals$expeta),
                            pred.mu = pred_vals$expeta$mean)
    pred_mu_b$pred.mu <- pred_mu_b$pred.mu/25
    ...
    scores2 <- data.frame(
      post_E = pred_vals$expeta$mean,
      SE_mu_B = (mu_B_centroid$mu - pred_vals$expeta$mean)^2,
      post_Var = pred_vals$expeta$sd^2)
    ...
    agg <- bru_mapper_logsumexp(rescale = FALSE, n_block = nrow(data$mu_B))
    
    pred_vals <- predict(fit_res$res, nested_grids_sf_all_centroid,
                         formula = ~ {
                           E <- exp(ibm_eval(mapper = agg,
                                             input = list(weights = weight, block = block),
                                             state = intercept + covariate))
                           list(
                             expeta = E
    ...
    scores2 <- data.frame(
      post_E = pred_vals$expeta$mean,
      SE_mu_B = (data$mu_B$mu - pred_vals$expeta$mean)^2,
      post_Var = pred_vals$expeta$sd^2)
  }
